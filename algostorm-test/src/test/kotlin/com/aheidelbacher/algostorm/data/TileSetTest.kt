...
class TileSetTest {
    ...
    private val stream = Resource("$SCHEMA/tileSet.json").inputStream()
    private val tileSet = tileSet {
        name = "world"
        tileWidth = 24
        tileHeight = 24
        image(Resource("$SCHEMA/testImage.png"), 288, 240)
        +tile {
            id = 1
            +Frame(1, 250)
            +Frame(2, 250)
    ...
    fun testTileSetDeserialization() {
        val actualTileSet = jsonDriver.readValue<TileSet>(stream)
